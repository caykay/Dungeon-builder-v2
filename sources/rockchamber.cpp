#include "../headers/rockchamber.h"
#include "../headers/rockwall.h"
#include <sstream>
...
std::string RockChamber::description() const
{
    ...
    while (count < (int)this->edges.size())
    {
        ...
        {
            description << nxt << intro << this->getDirection(count) << this->edges.at(static_cast<Direction>(count))->description() << nxt;
        }
        else
        {
            description << nxt << intro << this->getDirection(count) << this->edges.at(static_cast<Direction>(count))->description() << outro << nxt;
        }
        count++;
    }
